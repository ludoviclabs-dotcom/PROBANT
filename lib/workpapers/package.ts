...
export function buildWorkpaperPackage(snapshot: DossierSnapshot, scope: WorkpaperScope, actor: Principal | null) {
  ...
  if (!snapshot.workpaperProjection?.lockedRuns.length) throw new Error("LOCKED_EXPORT_REQUIRED");
  if (summary.rows.some((row) => row.stale)) throw new Error("STALE_REVIEW_EXPORT_BLOCKED");
