...
export const MODULE_MANIFEST: { id: string; label: string; status: Availability; limitation: string; production: false }[] = [
  { id: "documentation", label: "Documentation pédagogique locale", status: "disponible", limitation: "Guide de formation, pas une norme ni une méthode de mission", production: false },
  ...["Cash", "Cut-off", "Fournisseurs/RPNE", "Clients", "Immobilisations", "Capitaux propres", "Achats", "Congés payés", "Participations", "IS"].map((label, index) => ({ id: `cycle-${index}`, label, status: "démonstration" as const, limitation: "Cadre technique ; méthodes/PBC réelles non fournies. IS : planificateur et pont, pas calcul complet d’impôt.", production: false as const })),
  { id: "methods", label: "Méthodes applicables et PBC", status: "source requise", limitation: "Validation métier, périmètre, dates et pièces de la mission à obtenir", production: false },
  { id: "real", label: "Nouveaux parcours réels des feuilles de travail", status: "désactivé", limitation: "OIDC et ingestion durable existent sur main, mais ce nouveau parcours n’a pas encore d’adaptateur durable autorisé et validé. Aucune donnée réelle admise.", production: false },
  { id: "outside", label: "Stocks, ITGC, paie complète, DCF complet", status: "non implémenté", limitation: "Hors périmètre du programme accéléré ; les fiches historiques ne prouvent pas une implémentation", production: false },
];
