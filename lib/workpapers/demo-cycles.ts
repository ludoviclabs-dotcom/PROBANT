...
export function computeDemoCycle(p: DemoParameters, context: CycleContext, batch: ImportBatch, population: Population, selection: SelectionSet, actor: Principal) {
  ...
  if (p.scenario === "invalid") return { status: "invalid_input", reason: "Montant source invalide : contrôle non calculable ; aucune conversion en zéro." };
  ...
  switch (p.cycle) {
    case "cash": {
      ...
      return reconcileCash({ context, account: { id: "SYN-ACCOUNT", bankId: "SYN-BANK", bankLabel: "Banque synthétique", accountReference: "SYN-ACCOUNT", aliases: [], currency: "EUR" }, convention: { version: "synthetic-1", label: "positive_increases_book_balance", validatedBy: actor.id, bankColumn: "Bank", accountColumn: "Account" }, ledger: first, statement: p.scenario === "exception" ? second : first, erbBook: first, erbBank: p.scenario === "exception" ? second : first, items: [] });
    }
    case "cutoff": return analyzeCutoff({ ...cutoff("purchase"), recognition: { ...cutoff("purchase").recognition, alreadyRecognizedAmount: k(p.scenario === "exception" ? 0n : 10000n), bookingDate: context.period.closingDate } });
    case "fournisseurs": {
    ...
    case "conges": {
      ...
      return comparePaidLeave({ context, rights, first, second: p.scenario === "exception" ? { ...first, numerator: "1", denominator: "2" } : first, comparisonMethod: method, equivalenceEvidence: links, booked: null, associatedCharges: null });
    }
